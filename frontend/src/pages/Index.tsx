...
const Index = () => {
  ...
  const { processAudioFile, isProcessing, setIsProcessing } =
  ...
  const handleEnhanceFiles = useCallback(
    async (settings: any) => {
      setIsProcessing(true);
      const filesToProcess = audioFiles.filter(
      ...
      for (let i = 0; i < filesToProcess.length; i++) {
        ...
        try {
          ...
          const enhancedBlob = await processAudioFile(
            file,
            defaultSettings,
            (progress, stage) => {
              setAudioFiles((prev) =>
  ...
  const handleDownloadAll = async () => {
    const readyFiles = enhancedHistory.filter(
      (f) => f.status === "enhanced" && f.enhancedUrl,
    );
  ...
  const handleClearAll = useCallback(() => {
    ...
    if (
      window.confirm(
        `Are you sure you want to remove ALL ${audioFiles.length} files from the list?`,
      )
    ) {
      setAudioFiles([]);
      toast({
  ...
  const handleClearDownloaded = useCallback(() => {
    const downloadedFiles = enhancedHistory.filter(
      (f) => f.status === "enhanced",
    );
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 bg-blue-950">
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <Card className="bg-gradient-to-br from-slate-800 to-slate-900 border-slate-600 shadow-lg">
              <CardContent className="p-4 text-center bg-gray-500">
                <div className="text-2xl font-bold text-white bg-gray-500">
                  {stats.total}
                </div>
                <div className="text-sm text-slate-300 bg-gray-500">
                  Total Files
                </div>
              </CardContent>
            </Card>
