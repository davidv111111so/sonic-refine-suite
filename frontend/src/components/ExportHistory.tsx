...
export const ExportHistory = ({
  ...
  const mockHistory: EnhancementHistoryEntry[] =
    history.length === 0
      ? [
          {
            id: "1",
                  ...
                  <div key={item.id} className="bg-slate-700/50 rounded p-3">
                    ...
                    <div className="flex justify-between items-center mt-3">
                      <span className="text-xs text-slate-400">
                        ...
                        {formatFileSize(item.enhancedSize)}
                      </span>
          ...
          <TabsContent value="stats" className="mt-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <p className="text-2xl font-bold text-white">
                  {formatFileSize(
                    combinedHistory.reduce(
                      (total, item) => total + item.enhancedSize,
                      0,
                    ),
                  )}
                </p>
              ...
              <div className="bg-slate-700/50 rounded p-3 text-center">
                ...
                <p className="text-2xl font-bold text-white">
                  {Math.round(
                    (combinedHistory.filter((item) => item.status === "success")
                      .length /
                      combinedHistory.length) *
                      100,
                  )}
              ...
              <div className="bg-slate-700/50 rounded p-3 text-center">
                ...
                <p className="text-2xl font-bold text-white">
                  {Math.round(
                    (combinedHistory.reduce(
                      (total, item) =>
                        total + item.enhancedSize / item.originalSize,
                      0,
                    ) /
                      combinedHistory.length) *
                      100,
