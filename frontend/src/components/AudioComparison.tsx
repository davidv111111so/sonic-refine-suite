...
export const AudioComparison = ({
  ...
  const [currentAudio, setCurrentAudio] = useState<"original" | "enhanced">(
    "original",
  );
      ...
      if (originalAudioRef.current && enhancedAudioRef.current) {
        const currentTime =
          currentAudio === "original"
            ...
            : enhancedAudioRef.current.currentTime;
        const duration =
          currentAudio === "original"
            ? originalAudioRef.current.duration
            : enhancedAudioRef.current.duration;
    ...
    <Card className="bg-slate-800/50 border-slate-700">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <Badge
            variant={currentAudio === "original" ? "default" : "secondary"}
            onClick={() => handleAudioSwitch("original")}
            className="cursor-pointer"
          >
            Original
          </Badge>
          <Badge
            variant={currentAudio === "enhanced" ? "default" : "secondary"}
            onClick={() => handleAudioSwitch("enhanced")}
            className="cursor-pointer"
          >
            Enhanced
          </Badge>
