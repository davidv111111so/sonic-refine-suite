...
export const SpectrumTabs = ({
  ...
  const handleProcessingSettingChange = (
  ...
  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
      <TabsList className="grid w-full grid-cols-3 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 dark:from-black dark:via-slate-900 dark:to-black border-2 border-slate-600 dark:border-slate-700 p-1 rounded-xl shadow-xl">
        ...
        <TabsTrigger
          value="ai-mastering"
          className="flex items-center gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-cyan-500 data-[state=active]:via-purple-500 data-[state=active]:to-pink-500 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-purple-500/50 data-[state=active]:scale-105 transition-all duration-300 font-bold rounded-3xl"
        >
      ...
      <TabsContent value="spectrum" className="space-y-6">
        ...
        {(audioFiles.length > 0 || enhancedHistory.length > 0) && (
          <div className="flex justify-center gap-4 pt-6">
            {enhancedHistory.filter((f) => f.status === "enhanced").length >=
              2 && (
              <Button
          ...
          <CardHeader className="pb-4 bg-gray-800">
            <div className="flex items-center justify-between gap-4">
              ...
              <Button
                ...
                variant="spectrum"
                size="lg"
                className="shadow-xl shrink-0 rounded-xl text-4xl"
              >
              ...
              <div className="flex-1 text-center">
                ...
                <div className="flex items-center justify-center gap-4 px-6 py-3 bg-gradient-to-r from-purple-800/60 via-blue-800/60 to-green-800/60 dark:from-purple-900/70 dark:via-blue-900/70 dark:to-green-900/70 rounded-xl border-2 border-purple-400/50 shadow-lg">
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-gradient-to-r from-cyan-200 to-blue-200 bg-clip-text text-transparent font-bold">
                    ...
                    <span className="px-2 py-1 bg-purple-600/60 rounded-md text-sm font-bold border border-purple-400/60 shadow-lg text-neutral-50">
                      {audioFiles.length > 0
                        ? audioFiles[0].fileType?.toUpperCase() ||
                          audioFiles[0].name.split(".").pop()?.toUpperCase() ||
                          "N/A"
                        : "-"}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-gradient-to-r from-pink-200 to-purple-200 bg-clip-text text-transparent font-bold">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-gradient-to-r from-blue-200 to-cyan-200 bg-clip-text text-transparent font-bold">
                      {t("enhance.quality")}:
                    </span>
                    <span className="px-2 py-1 bg-blue-600/60 rounded-md text-sm font-bold border border-blue-400/60 shadow-lg text-neutral-50">
                      {processingSettings.sampleRate / 1000}kHz{" "}
                      {processingSettings.bitDepth}bit
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-gradient-to-r from-slate-200 to-gray-200 bg-clip-text text-transparent font-bold">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-gradient-to-r from-green-200 to-emerald-200 bg-clip-text text-transparent font-bold">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <DynamicOutputSettings
            ...
            onOutputFormatChange={(format) =>
              handleProcessingSettingChange("outputFormat", format)
            }
            onSampleRateChange={(rate) =>
              handleProcessingSettingChange("sampleRate", rate)
            }
            onBitDepthChange={(depth) =>
              handleProcessingSettingChange("bitDepth", depth)
            }
            onBitrateChange={(rate) =>
              handleProcessingSettingChange("bitrate", rate)
            }
          />
          ...
          <InteractiveProcessingOptions
            ...
            onNoiseReductionChange={(value) =>
              handleProcessingSettingChange("noiseReduction", value)
            }
            onNoiseReductionEnabledChange={(enabled) =>
              handleProcessingSettingChange("noiseReductionEnabled", enabled)
            }
            onNormalizeChange={(enabled) =>
              handleProcessingSettingChange("normalize", enabled)
            }
            onNormalizeLevelChange={(level) =>
              handleProcessingSettingChange("normalizeLevel", level)
            }
            onCompressionChange={(value) =>
              handleProcessingSettingChange("compression", value)
            }
            onCompressionEnabledChange={(enabled) =>
              handleProcessingSettingChange("compressionEnabled", enabled)
            }
            onStereoWideningChange={(value) =>
              handleProcessingSettingChange("stereoWidening", value)
            }
            onStereoWideningEnabledChange={(enabled) =>
              handleProcessingSettingChange("stereoWideningEnabled", enabled)
            }
