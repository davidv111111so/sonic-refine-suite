  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      ...
      if (!hasConsented) {
        ...
        setErrorMessage(
          "Please accept the Terms and Conditions before uploading files.",
        );
        return;
      }
      ...
      if (file) {
        if (file.errors && file.errors.length > 0) {
          ...
          if (error.code === "file-too-large") {
            setErrorMessage(
              `File "${file.file.name}" is too large. Maximum size is ${Math.round(maxFileSize / 1024 / 1024)}MB for optimal processing.`,
            );
          } else if (error.code === "file-invalid-type") {
            setErrorMessage(
              `File "${file.file.name}" format not supported. Supported formats: ${supportedFormats.join(", ")}.`,
            );
          } else {
            setErrorMessage(
            ...
            {isDragActive && hasConsented ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm text-sky-300">
                  {supportedFormats.join(", ").toUpperCase()} (Max{" "}
                  {Math.round(maxFileSize / 1024 / 1024)}MB each, {maxFiles}{" "}
                  files)
                </p>
              </div>
            )}
          ...
          <div className="mt-6 pt-4 border-t border-slate-600">
            <div className="flex items-start gap-3">
              <Checkbox
                ...
                className="mt-1 bg-gray-50 text-blue-950"
              />
              <div className="flex-1">
                <label
                  ...
                  style={{
                    textShadow:
                      "0 0 20px rgba(96, 165, 250, 0.4), 0 0 30px rgba(165, 180, 252, 0.3)",
                  }}
                >
                  I agree to the{" "}
                  <Link
                  ...
                  and acknowledge the Copyright Disclaimer. I confirm that I own
                  or have proper authorization for all audio files I upload and
