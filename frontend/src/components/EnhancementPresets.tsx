...
export const EnhancementPresets = ({
  ...
  const getPresetDescription = (preset: Preset) => {
    const desc = [];
    if (preset.settings.bassBoost > 0)
      desc.push(`+${preset.settings.bassBoost}dB Bass`);
    if (preset.settings.midBoost > 0)
      desc.push(`+${preset.settings.midBoost}dB Mid`);
    if (preset.settings.trebleBoost > 0)
      desc.push(`+${preset.settings.trebleBoost}dB Treble`);
    if (preset.settings.noiseReduction > 0)
      desc.push(`${preset.settings.noiseReduction}% Noise Reduction`);
    if (preset.settings.compression > 0)
      desc.push(`${preset.settings.compression}% Compression`);
    if (preset.settings.stereoWidening > 0)
      desc.push(`${preset.settings.stereoWidening}% Stereo Width`);
    return desc.join(", ");
  };
