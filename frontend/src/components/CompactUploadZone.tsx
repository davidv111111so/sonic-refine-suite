  ...
  const onDrop = useCallback(
    (acceptedFiles) => {
      ...
      if (validFiles.length !== acceptedFiles.length) {
        setErrorMessage(
          "Some files were rejected. Perfect Audio v2.0 only supports MP3 and WAV formats.",
        );
      }
      ...
      if (file) {
        if (file.errors && file.errors.length > 0) {
          ...
          if (error.code === "file-too-large") {
            setErrorMessage(
              `File "${file.file.name}" is too large. Maximum size is 100MB for optimal processing.`,
            );
          } else if (error.code === "file-invalid-type") {
            setErrorMessage(
              `File "${file.file.name}" format not supported. Perfect Audio v2.0 supports MP3 and WAV only.`,
            );
          } else {
            setErrorMessage(
    ...
    <div className="space-y-4">
      <Card className="bg-gradient-to-br from-slate-800 to-slate-900 border-slate-600 shadow-lg">
        <CardContent className="p-3">
          <div
            {...getRootProps()}
            className="flex flex-col items-center justify-center h-24 border-2 border-dashed rounded-md cursor-pointer bg-slate-700/20 border-slate-500/50 hover:bg-slate-700/40 hover:border-blue-400/50 transition-all duration-200"
          >
            <input {...getInputProps()} />
            {isDragActive ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="text-white text-sm text-center font-medium">
                  Drag audio files here or click to select (Max 20 files)
                </p>
                <p className="text-xs text-slate-300">
                  MP3, WAV (Max 100MB each)
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-slate-400">
              Total:{" "}
              {formatFileSize(
                displayFiles.reduce((sum, file) => sum + file.size, 0),
              )}
            </div>
          </div>
          ...
          {uploadedFiles.length > 20 && (
            <div className="text-center text-xs text-slate-400 p-2 bg-slate-800/50 rounded">
              Showing last 20 of {uploadedFiles.length} files. Older files are
