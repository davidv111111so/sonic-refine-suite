      ...
      if (file) {
        if (file.errors && file.errors.length > 0) {
          ...
          if (error.code === "file-too-large") {
            setErrorMessage(
              `File "${file.file.name}" is too large. Maximum size is 200MB.`,
            );
          } else if (error.code === "file-invalid-type") {
            setErrorMessage(
              `File "${file.file.name}" has an invalid file type. Only audio files are allowed.`,
            );
          } else {
            setErrorMessage(
  ...
  return (
    <Card className="bg-slate-800/50 border-slate-700">
      <CardContent className="p-4">
        <div
          {...getRootProps()}
          className="flex flex-col items-center justify-center h-48 border-2 border-dashed rounded-md cursor-pointer bg-slate-700/20 border-slate-500/50 hover:bg-slate-700/40 transition-colors"
        >
          <input {...getInputProps()} />
          {isDragActive ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Supported formats: mp3, wav, flac, m4a (Max 200MB)
              </p>
            </div>
          )}
        </div>
