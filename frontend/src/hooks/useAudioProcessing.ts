      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          const audioBuffer = (await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error("Audio decode timeout")), 8000),
            ),
          ])) as AudioBuffer;
          ...
          const enhancedBuffer = await processAudioInChunks(
            audioBuffer,
            audioContext,
            settings,
            onProgressUpdate,
          ...
          const enhancedArrayBuffer = await encodeAudioBuffer(
            enhancedBuffer,
            settings,
          );
...
const processAudioInChunks = async (
  ...
  const enhancedLength = Math.ceil(
  ...
  for (let channel = 0; channel < numberOfChannels; channel++) {
    ...
    for (let start = 0; start < enhancedLength; start += chunkSize) {
      ...
      if (onProgressUpdate && start % chunkSize === 0) {
        ...
        onProgressUpdate(
          progress,
          `Processing channel ${channel + 1}/${numberOfChannels}...`,
        );
