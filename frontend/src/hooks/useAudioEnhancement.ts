...
export const useAudioEnhancement = (
  ...
  const requestBulkDownloadAuthorization = async (fileCount: number) => {
    ...
    const confirmed = confirm(
      `Download ${fileCount} enhanced files? They will be saved to your Downloads folder.`,
    );
    if (confirmed) {
      setBulkDownloadAuthorized(true);
    }
  ...
  const handleEnhanceFiles = useCallback(
    async (settings: any) => {
      ...
      if (filesToProcess.length > 1) {
        dirHandle = await requestBulkDownloadAuthorization(
      ...
      if (downloadQueue.length > 1 && bulkDownloadAuthorized) {
        ...
        const downloaded = await downloadFile(
          downloadQueue[0].blob,
          downloadQueue[0].filename,
          folderName,
          dirHandle,
        );
        if (downloaded) successfulDownloads++;
  ...
  const handleDownloadAllAsZip = useCallback(async () => {
    const enhancedFiles = audioFiles.filter(
      (file) => file.status === "enhanced" && file.enhancedUrl,
    );
    ...
    try {
      ...
      const folder = zip.folder(
        `Spectrum_Enhanced_${new Date().toISOString().slice(0, 10)}`,
      );
