      ...
      return new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              try {
                ...
                throw new Error(
                  `Audio processing failed: ${decodeError.message || "Unable to decode audio data"}`,
                );
              }
            } else {
              throw new Error(
          ...
          if (
            ...
            currentNode = applyNoiseReduction(
              offlineContext,
              currentNode,
              settings.noiseReduction,
            );
          ...
          if (settings.normalize) {
            currentNode = applyNormalization(
              offlineContext,
              currentNode,
              settings.normalizeLevel || -3,
            );
          }

          if (
            settings.stereoWideningEnabled &&
            settings.stereoWidening &&
            settings.stereoWidening > 0
          ) {
            currentNode = applyStereoWidening(
              offlineContext,
              currentNode,
              settings.stereoWidening,
            );
          ...
          const trebleBoostValue =
            settings.trebleEnhancement || settings.trebleBoost || 0;

          if (bassBoostValue !== 0 && isFinite(bassBoostValue)) {
            currentNode = applyBassBoost(
              offlineContext,
              currentNode,
              bassBoostValue,
            );
          ...
          const blob = await audioBufferToBlob(
            renderedBuffer,
