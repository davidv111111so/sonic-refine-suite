...
export const useFileManagement = () => {
  ...
  useEffect(() => {
    const loadStoredFiles = () => {
      try {
        ...
        const uploadedFiles = parsedData.files.filter(
          (file) => file.status === "uploaded" && file.name && file.size,
        );
        ...
        const restoredFiles: AudioFile[] = uploadedFiles.map((file) => ({
          ...
          originalFile: new File([], file.name || "unknown", {
            type: file.type || "audio/*",
          }),
          artist: file.artist || "Unknown Artist",
          title:
            file.title || file.name?.replace(/\.[^.]+$/, "") || "Unknown Title",
        }));
  ...
  const handleUpdateFile = useCallback(
    (fileId: string, updates: Partial<AudioFile>) => {
      setAudioFiles((prev) =>
        prev.map((file) =>
