...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    const enhancedBuffer = await applyRealAudioEnhancements(
      audioBuffer,
      settings,
      fileId,
    );
    ...
    const metadata = {
      ...
      format: "WAV 24-bit",
      enhancements: Object.keys(settings).filter(
        (key) => settings[key] && settings[key] !== 0,
      ),
    };
...
// Apply real audio enhancements
async function applyRealAudioEnhancements(
  ...
  for (let channel = 0; channel < channels; channel++) {
    ...
    await applyChannelEnhancements(
      outputData,
      settings,
      sampleRate,
      fileId,
...
async function applyChannelEnhancements(
