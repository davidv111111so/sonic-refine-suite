...
export const generateFFmpegCommand = (
  ...
  if (settings.noiseReduction) {
    const noiseReduction = Math.min(settings.noiseReductionLevel / 100 * 30, 30); // Max 30dB reduction
  ...
  if (settings.enableEQ && settings.eqBands.some(band => band !== 0)) {
    ...
    const eqFilters = settings.eqBands
      .map((gain, i) => gain !== 0 ? `equalizer=f=${eqFreqs[i]}:width_type=h:width=200:g=${gain}` : null)
      .filter(Boolean);
  ...
  filters.push('alimiter=level_in=1:level_out=0.95:limit=0.95:attack=5:release=50');
...
export const validateEnhancementSettings = (settings: EnhancementSettings): string[] => {
  const issues: string[] = [];
  
  if (settings.targetBitrate < 128) {
    issues.push('Bitrate too low for quality enhancement (minimum 128 kbps recommended)');
  }
  
  if (settings.sampleRate < 44100) {
    issues.push('Sample rate too low for quality enhancement (minimum 44.1 kHz recommended)');
  }
  
  if (settings.noiseReductionLevel > 80) {
    issues.push('High noise reduction may cause artifacts (recommended: 50-70%)');
  }
  
  if (Math.abs(settings.gainAdjustment) > 12) {
    issues.push('Excessive gain adjustment may cause distortion (recommended: ±6dB)');
  }
