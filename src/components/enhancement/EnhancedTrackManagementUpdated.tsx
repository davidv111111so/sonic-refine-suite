...
export const EnhancedTrackManagement = ({
  ...
  const allFiles = useMemo(() => {
    return [...audioFiles, ...enhancedHistory].filter(file => !removedFileIds.has(file.id));
  ...
  const hasEnhancedFiles = enhancedHistory.some(file => file.status === 'enhanced' && !removedFileIds.has(file.id));
          ...
          <div className="mt-6 pt-4 border-t border-slate-600 dark:border-slate-700">
            <div className="flex justify-between items-center mb-4">
              <div className="flex gap-3">
                {/* Download All Button */}
                {enhancedHistory.filter((f) => f.status === 'enhanced' && !removedFileIds.has(f.id)).length >= 2 && (
                  <Button
                  ...
                  >
                    ...
                    Download All ({enhancedHistory.filter((f) => f.status === 'enhanced' && !removedFileIds.has(f.id)).length})
                  </Button>
                )}

                {/* Clear Button */}
                {enhancedHistory.filter(f => !removedFileIds.has(f.id)).length > 0 && onClearDownloaded && (
                  <Button
                  ...
                  >
                    ...
                    Clear Downloaded ({enhancedHistory.filter(f => !removedFileIds.has(f.id)).length})
                  </Button>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-4 text-center">
              <div className="rounded-lg p-3 border border-slate-600/50 bg-slate-600">
                ...
                <div className="text-xl font-bold text-white">{sortedFiles.length}</div>
              </div>
