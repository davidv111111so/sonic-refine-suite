  ...
  return <Card className="bg-slate-900/90 dark:bg-black/90 border-slate-700 dark:border-slate-800">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 bg-gradient-to-r from-purple-200 via-pink-200 to-blue-200 bg-clip-text font-bold text-lg bg-gray-900 hover:bg-gray-800 text-red-500">
      ...
      <CardContent className="pt-0 space-y-6">
        ...
        <div className="space-y-3">
          ...
          {normalize && <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-xs bg-gradient-to-r from-slate-200 to-gray-200 bg-clip-text font-semibold text-amber-400">Target Level</span>
                <span className="text-xs text-white font-mono font-bold">{normalizeLevel} dB</span>
              </div>
              <Slider value={[normalizeLevel]} onValueChange={([value]) => onNormalizeLevelChange(value)} min={-12} max={0} step={0.1} className="w-full" />
            </div>}
        </div>
        ...
        <div className="space-y-3">
          ...
          {compressionEnabled && <div className="space-y-2">
              <div className="flex justify-between">
              ...
              <Slider value={[compression]} onValueChange={([value]) => onCompressionChange(value)} min={1} max={10} step={0.5} className="w-full" />
            </div>}
        </div>
        ...
        <div className="space-y-3">
          ...
          {stereoWideningEnabled && <div className="space-y-2">
              ...
              <Slider value={[stereoWidening]} onValueChange={([value]) => onStereoWideningChange(value)} min={0} max={100} step={1} className="w-full" />
            </div>}
        </div>
      </CardContent>
    </Card>;
