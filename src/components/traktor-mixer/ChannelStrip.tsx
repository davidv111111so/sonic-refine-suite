...
export const ChannelStrip = ({ deck, color, label, side, cue, onToggleCue }: ChannelStripProps) => {
    ...
    return (
        <div className={cn(
            "flex flex-col h-full bg-[#121212] px-1 py-2 relative gap-2 items-center min-w-[60px]",
            ...
            <button
                className={cn(
                    "w-8 h-6 rounded-sm border flex items-center justify-center transition-all mt-1",
                    cue
            ...
            {/* Volume Fader */}
            <div className="flex-1 w-full flex justify-center relative min-h-[120px]">
                <Fader
            ...
            <div className={cn("font-bold text-[10px] tracking-widest text-center mt-1", labelColor)}>
                {label}
            </div>
        </div>
    );
