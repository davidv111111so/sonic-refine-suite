...
export const AdminReferenceManager: React.FC = () => {
  ...
  if (loading) {
    return <Card className="bg-slate-900 border-slate-700">
          ...
          return <Card key={genre} className="bg-slate-800/50 border-slate-700">
                <CardContent className="p-4">
                  <div className="space-y-3">
                    ...
                    <label>
                      <input type="file" accept="audio/*" className="hidden" disabled={isUploading} onChange={e => {
                    const file = e.target.files?.[0];
                    if (file) handleUpload(genre, file);
                    e.target.value = '';
                  }} />
                      <Button variant="outline" className="w-full bg-slate-700 border-slate-600 hover:bg-slate-600 text-white" disabled={isUploading} asChild>
                        <span>
                          {isUploading ? <>
