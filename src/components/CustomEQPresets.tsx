...
export const CustomEQPresets = ({ currentEQBands, onApplyPreset }: CustomEQPresetsProps) => {
  ...
  useEffect(() => {
    localStorage.setItem('perfectAudio_customEQPresets', JSON.stringify(customPresets));
  }, [customPresets]);
      ...
      <CardContent className="pt-0 space-y-4">
        ...
        {customPresets.length > 0 ? (
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-slate-300">Your Custom Presets</h4>
            {customPresets.map((preset) => (
              <div key={preset.id} className="flex items-center justify-between p-2 bg-slate-800/50 rounded border border-slate-700">
          ...
          <div className="text-center py-4">
            ...
            <p className="text-slate-500 text-xs">Create your first custom EQ preset above</p>
          </div>
        )}

        {customPresets.length > 0 && (
          <div className="text-xs text-slate-500 text-center pt-2 border-t border-slate-700">
            {customPresets.length} custom preset{customPresets.length !== 1 ? 's' : ''} saved locally
          </div>
        )}
      </CardContent>
    </Card>
