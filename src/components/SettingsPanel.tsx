    ...
    <Card className="bg-slate-800/50 border-slate-600">
      ...
      <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => handleSettingChange('outputFormat', e.target.value as 'mp3' | 'wav')}
                disabled={disabled}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <label className="text-xs text-slate-300">Audio Normalization</label>
            <Switch
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs text-slate-300">Compression Ratio</label>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400">{settings.stereoWidth}%</span>
            </div>
            <Slider
              value={[settings.stereoWidth]}
              onValueChange={([value]) => handleSettingChange('stereoWidth', value)}
              max={100}
              step={1}
              disabled={disabled}
              className="w-full"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400">{settings.bassBoost > 0 ? '+' : ''}{settings.bassBoost}dB</span>
            </div>
            <Slider
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs text-slate-300">Treble Enhancement</label>
              <span className="text-xs text-slate-400">{settings.trebleEnhancement > 0 ? '+' : ''}{settings.trebleEnhancement}dB</span>
            </div>
            <Slider
              value={[settings.trebleEnhancement]}
              onValueChange={([value]) => handleSettingChange('trebleEnhancement', value)}
              min={-12}
              max={12}
              step={0.5}
              disabled={disabled}
        ...
        {track && (
          <div className="pt-4 border-t border-slate-600">
            <h4 className="text-sm font-medium text-white mb-2">Track Information</h4>
            <div className="space-y-1 text-xs text-slate-400">
              <div>Status: <span className="text-white">{track.status}</span></div>
              {track.metadata && (
                <>
                  <div>Duration: <span className="text-white">{Math.round(track.metadata.duration)}s</span></div>
                  <div>Sample Rate: <span className="text-white">{track.metadata.sampleRate}Hz</span></div>
                  <div>Channels: <span className="text-white">{track.metadata.channels}</span></div>
                </>
              )}
            </div>
          </div>
