  ...
  const getSliderBackground = (index: number, value: number) => {
    ...
    return {
      background: `linear-gradient(180deg, 
        ${color}${Math.floor(opacity * 255).toString(16).padStart(2, '0')} 0%, 
        ${color}40 50%, 
        transparent 100%)`,
      boxShadow: value !== 0 ? `0 0 20px ${color}60, inset 0 0 10px ${color}20` : 'none',
      borderRadius: '8px',
            ...
            <div key={freq} className="flex flex-col items-center group">
              ...
              <div 
                ...
                style={{
                  ...
                  borderColor: eqBands[index] !== 0 ? getEQColor(index, eqBands[index]) + '60' : '#475569',
                  boxShadow: eqBands[index] !== 0 ? `0 0 15px ${getEQColor(index, eqBands[index])}40` : 'none'
                }}
              >
