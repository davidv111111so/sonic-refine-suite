    ...
    <div className="space-y-4">
      <Card className="bg-gradient-to-br from-slate-800 to-slate-900 border-slate-600 shadow-lg">
        <CardHeader className="pb-3">
          <CardTitle className="text-white text-lg flex items-center gap-3">
            ...
            <span className="text-sm text-slate-400 font-normal">- Last 20 Enhanced</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-0">
          {displayFiles.length > 0 ? (
              ...
              {displayFiles.map((file) => (
                <div key={file.id} className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg border border-slate-700">
                      ...
                      <div className="flex items-center gap-4 text-xs text-slate-400 mt-1">
                        ...
                        {file.enhancedSize && (
                          <>
                            ...
                            <span className={
                              getCompressionRatio(file.size, file.enhancedSize).startsWith('+') 
                                ? 'text-blue-400' 
                                : 'text-green-400'
                            }>
                              {getCompressionRatio(file.size, file.enhancedSize)}
                            </span>
                          </>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setCompareFile(compareFile?.id === file.id ? null : file)}
                  ...
                  {compareFile?.id === file.id && (
                    <AudioComparison
                      originalUrl={file.originalFile ? URL.createObjectURL(file.originalFile) : undefined}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-slate-500 text-sm">Process songs in the Enhance tab to see them here</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      {enhancedFiles.length > 20 && (
        <div className="text-center text-xs text-slate-400 p-2 bg-slate-800/50 rounded">
          ...
          <span className="text-blue-400 ml-1">Buy Pro version for unlimited history!</span>
        </div>
      )}
    </div>
  );
