    ...
    <Card className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-slate-700 p-6 shadow-2xl">
      ...
      <div className="grid grid-cols-8 gap-3 bg-slate-950/50 p-6 rounded-xl border border-slate-700/50 relative overflow-hidden">
        ...
        >
          ...
          {/* Zone labels */}
          <text x="5%" y="95%" fill="#06b6d4" fontSize="8" opacity="0.5">
            SUB
          </text>
          <text x="20%" y="95%" fill="#8b5cf6" fontSize="8" opacity="0.5">
            BASS
          </text>
          <text x="40%" y="95%" fill="#a855f7" fontSize="8" opacity="0.5">
            MID
          </text>
          <text x="65%" y="95%" fill="#d946ef" fontSize="8" opacity="0.5">
            HIGH
          </text>
          <text x="85%" y="95%" fill="#ec4899" fontSize="8" opacity="0.5">
            AIR
          </text>
        </svg>
        {bands.map((band, index) => (
          ...
          >
            ...
            <div
              className={`text-xs font-mono font-bold px-3 py-2 rounded-lg min-w-[65px] text-center transition-all ${
                band.gain > 0
        ...
        <div className="h-20 relative flex items-end justify-around z-10">
          {bands.map((band, index) => {
            const height = Math.max(
              0,
              Math.min(100, 50 + (band.gain / 12) * 40)
            );
            const color =
              index < 2
            ...
            return (
              <div
                key={index}
                className={`w-full mx-0.5 bg-gradient-to-t ${color} rounded-t transition-all duration-300 shadow-lg`}
                style={{
                  height: `${height}%`,
                  boxShadow: `0 0 10px ${
                    index < 2
