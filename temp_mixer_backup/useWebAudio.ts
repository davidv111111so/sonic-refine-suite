...
import AudioWorker from '../workers/audioDecoder.worker?worker';
...
export const useWebAudio = () => {
    ...
    useEffect(() => {
        ...
        const cueMixNode = ctx.createGain();   // Feeds Master into Headphones
        ...
        // Headphone Volume -> Destination (Simulated separate output)
    ...
    const loadTrack = async (deck: DeckControls, file: File | string) => {
        ...
        try {
            ...
            deck.loadTrack(audioBuffer, detectedBPM);
        } catch (err) {
    ...
    const wrapDeck = (deck: DeckControls) => ({
        ...deck,
        loadTrack: (file: File | string) => loadTrack(deck, file)
    });
