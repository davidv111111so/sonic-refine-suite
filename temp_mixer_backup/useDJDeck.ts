...
export interface DeckControls {
    ...
    loadTrack: (source: AudioBuffer | File | string, bpm?: number) => void | Promise<void>;
...
export const useDJDeck = (context: AudioContext | null): DeckControls => {
    ...
    // Initialize FX Chain
    // We need to pass the nodes to the hook, but the nodes are created in useEffect.
    // However, useGroupFXChain expects nodes to be passed or it can create them?
    // The hook signature is: (audioContext, sourceNode, destinationNode)
    // We can create stable "Anchor" nodes in a ref or state to pass to the hook.
    ...
    useEffect(() => {
        ...
        // 2. Connect Graph
        // Source -> Trim -> EQ High -> Mid -> Low -> FilterHP -> FilterLP -> FX Input
        trim.connect(high);
        high.connect(mid);
        mid.connect(low);
        ...
        // Analyser will be the Master Output exposed

        // Path 2: Cue (Splitter -> Cue Gate -> Output)
        splitter.connect(cueGate);
        // Cue Gate will be the Cue Output exposed
    ...
    }, [state.playbackRate, state.keyLock]);
    ...
    const loadTrack = useCallback((buffer: AudioBuffer, bpm?: number) => {
        setState(prev => ({
            ...prev,
            buffer,
            stems: null,
    ...
    return {
        play, pause, cue, seek, setSpeed, setVolume, setTrim, setPitch, setEQ, toggleEQKill, setFilter, setStemVolume,
        toggleLoop, loopIn, loopOut, loopShift, setLoopPoints, loadTrack, loadStems,
        setKeyLock,
        state,
